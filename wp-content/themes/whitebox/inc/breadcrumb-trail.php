<?php
/**
 * Adapted from the code of Justin Tadlock
 * @package whitebox
 * @since 1.0
 ...
 */
function whitebox_breadcrumb_trail( $args = array() ) {

	if ( function_exists( 'is_bbpress' ) && is_bbpress() )
		$breadcrumb = new bbPress_Breadcrumb_Trail( $args );
	else
		$breadcrumb = new Whitebox_Breadcrumb_Trail( $args );
 ...
 */
class Whitebox_Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$br_ss_browse = __("You are here: ", "whitebox");
			$breadcrumb .= "\n\t\t\t" . '<nav class="trail-nav"><span class="trail-browse">' . $br_ss_browse  . '</span> ';
		...
		$breadcrumb = apply_filters( 'whitebox_breadcrumb_trail', $breadcrumb, $this->args );
	 ...
	 */
	public function whitebox_get_post_types_by_slug( $slug ) {
			...
			if ( $taxonomy->rewrite['slug'] ) {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						$post_types = $this->whitebox_get_post_types_by_slug( $match );
 ...
 */
class bbPress_Breadcrumb_Trail extends WhiteBox_Breadcrumb_Trail {
